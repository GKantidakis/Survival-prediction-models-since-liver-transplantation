...
var_imp_keras <- function(model){
  ...
  # extract the column names from created data training_ffs_long: the covariates 1:195
  # and additional the 10 time variables times.1 to times.10
  names <- c(colnames(training_ffs_long)[1:195], colnames(training_ffs_long)[202:211]) 
...
indexa <- 2:length(timing)
int_bs <- (diff(timing) %*% ((PredError$AppErr$coxph[indexa - 1] 
                              + PredError$AppErr$coxph[indexa])/2))/diff(range(timing))
...
install_packages <- c("survival", "glmnet", "hdnom", "caret")
...
####################################################################
# prepare the data in one-hot encoding standardized format
####################################################################

install_packages <- c("data.table", "mltools")
...
# creating training and test ffs scaled

response_pair <- c("gs_ffs", "gs_ffsstate")

# for the training set
X_train <- training_ffs[, !(colnames(training_ffs) %in% response_pair)]
X_train_scaled <- X_train # duplicate data
...
X_train_ffs <- one_hot(as.data.table(X_train_scaled))
# create martix X for glmnet
# dim(X_train_ffs) # 41529 x 128
Y_train_ffs <- training_ffs[, colnames(training_ffs) %in% response_pair] # create matrix Y


train_ffs_df <- as.data.frame(X_train_ffs)

training_ffs_scaled_1hot <- as.data.frame(cbind(train_ffs_df, Y_train_ffs))
save(training_ffs_scaled_1hot, file = "training_ffs_scaled_1hot.RData")
...
X_test_ffs <- as.data.frame(one_hot(as.data.table(X_test_scaled)))
Y_test_ffs <- test_ffs[, colnames(test_ffs) %in% response_pair]

test_ffs_scaled_1hot <- as.data.frame(cbind(X_test_ffs, Y_test_ffs))
save(test_ffs_scaled_1hot, file = "test_ffs_scaled_1hot.RData")
...
install_packages <- c("fastDummies", "keras", "survival",
                      "pec", "caret")
...
load("training_ffs_scaled_1hot.RData")
load("test_ffs_scaled_1hot.RData")
load("training_ffs_long.RData")
load("test_ffs_long.RData")

# training_ffs_long <- data_train_creator(data = training_ffs_scaled_1hot)
# test_ffs_long <- data_test_creator(data = test_ffs_scaled_1hot)
# save(training_ffs_long, file = "training_ffs_long.RData")
# save(test_ffs_long, file = "test_ffs_long.RData")

# table(training_ffs_long$status) / nrow(training_ffs_long) # class 0: 0.934, class1: 0.066
...
nfolds <- 5
set.seed(1234)
folds <- createFolds(training_ffs_scaled_1hot$gs_ffs, k = 5, list = TRUE)

node_size <- seq(20, 200, by = 30) # grid of node sizes
dropout_rate <- c(0.1, 0.2, 0.3)
lr <- c(0.01, 0.1, 0.2)
class_weights <- c(1, 10) # increase the weight of weak class 10 times to account the 10 intervals
momentum <- c(0.8, 0.9)
...
measures_calculator <- function(trained_model,
  ...
  df1 <- data.frame(hazard = predict_proba(trained_model,
                                           as.matrix(datanew[, c(1:195, 202:211)]),
                                           batch_size = 1000))
...
for (i in 1:nfolds) {
  ...
  train_set <-  data_train_creator(training_ffs_scaled_1hot[-indices, ]) # create the train set
  cat("Creating the validation set ...", "\n")
  validation_set <- data_test_creator(data = training_ffs_scaled_1hot[indices, ]) # create the validation set
  # real status (if a patient survived or died) for the validation set
  event_status <- training_ffs_scaled_1hot[indices, ]$gs_ffsstate 
  
  # create the matrices to be used for keras library
  train_x <- as.matrix(train_set[, c(1:195, 202:211)]) # predictors: 195 variables + intervals
  dimnames(train_x) <- NULL # the object must have empty dimnames
  train_y <- train_set$status
  validation_x <- as.matrix(validation_set[, c(1:195, 202:211)])
  ...
  for (j in 1:nrow(combis)) {
    ...
    # we create a densely connected ANN to the output (input shape 195 + 10)
    fit_keras %>%
      layer_dense(units = combis[j, 1], activation = 'sigmoid', input_shape = c(205)) %>% 
...
df <- data.frame(name = colnames(training_ffs_long)[c(1:195, 202:211)],
                 var_imp = rel_imp)
df <- df[order(df$var_imp, decreasing = TRUE), ]
rownames(df) <- NULL

# separately for variables without time intervals
df2 <- df[!df$name %in% colnames(training_ffs_long)[202:211], ]
df2 <- df2[order(df2$var_imp, decreasing = TRUE), ]
rownames(df2) <- NULL
head(df2, 10)


df3 <- df[df$name %in% colnames(training_ffs_long)[202:211], ]
...
fit_keras %>%
  layer_dense(units = 110, activation = 'sigmoid', input_shape = c(205)) %>% 
...
event_status <- test_ffs_scaled_1hot$gs_ffsstate

# create the matrices to be used for keras library
train_x <- as.matrix(training_ffs_long[, c(1:195, 202:211)]) 
dimnames(train_x) <- NULL # the object must have empty dimnames
train_y <- training_ffs_long$status
test_x <- as.matrix(test_ffs_long[, c(1:195, 202:211)])
...
fit_keras2 %>%
  layer_dense(units = 140, activation = 'sigmoid', input_shape = c(205)) %>% 
  layer_dropout(rate = 0.2) %>%
  layer_dense(units = 140, activation = 'sigmoid') %>%
  layer_dropout(rate = 0.2) %>%
  layer_dense(units = 1, activation = 'sigmoid')
fit_keras2 %>% compile(
  loss = 'binary_crossentropy', # for binary class classification problem
...
# create the matrices to be used for keras library
train_x <- as.matrix(training_ffs_long[, c(1:195, 202:211)]) # predictors: 195 variables + interval
dimnames(train_x) <- NULL # the object must have empty dimnames
train_y <- training_ffs_long$status
test_x <- as.matrix(test_ffs_long[, c(1:195, 202:211)])
dimnames(test_x) <- NULL # the object must have empty dimnames
...
# separately for variables without time intervals
dfb2 <- dfb[!dfb$name %in% colnames(training_ffs_long)[202:211], ]
...
dfb3 <- dfb[dfb$name %in% colnames(training_ffs_long)[202:211], ]
...
freqb <- lapply(vars, function(x) most_common(X_train_scaled[, x]))
...
#coefficients from the Cox model sorted by Z-score value
round(res_nolist[order(res_nolist[, 4], decreasing = T), ][1:20, ], digits = 3)
...
# df2 <- rbind(brier_cox_full$Brier, brier_cox_back$Brier, brier_cox_lasso$Brier,
#              brier_rsf, brier_nn1h, brier_nn2h)
...
# candidate variables are retransplantation, donorage and diabort
...
df_synth_scaled <- df_synth_scaled[1:3, ]

df_synth_normal$diab <- combis$diab
df_synth_normal$retransplantation <- combis$retransplantation

df_synth_scaled$diab <- combis$diab
df_synth_scaled$retransplantation <- combis$retransplantation
...
df_synth_scaled <- as.data.frame(one_hot(as.data.table(df_synth_scaled)))

df_synth_scaled$gs_ffs <- c(1, 5, 7)
df_synth_scaled$gs_ffsstate <- c(0, 1, 0)
...
df_new <- data.frame(hazard = predict_proba(fit_keras,
                                            as.matrix(data_synth_long[, c(1:195, 202:211)]),
                                            batch_size = 1000))
...
legend("bottomleft", legend = c("Cox", "RSF", "NN", 
       ...
       lty = c(1, 3, 2, 1, 1, 1) , col = c("black", "black", "black",
                                           "green", "orange", "red"),
       bty = "n", cex = 0.9, border = "white", xjust = 0)


# extra checks for the neural network
var_imp <- var_imp_keras(model = fit_keras)
...
test_scaled_try <- test_ffs_scaled_1hot[c(4, 10000, 20000), ]
...
legend("bottomleft", legend = c("Cox", "RSF", "NN", 
       ...
       lty = c(1, 3, 2, 1, 1, 1) , col = c("black", "black", "black",
                                           "green", "orange", "red"),
       bty = "n", cex = 0.8, border = "white", xjust = 0)
