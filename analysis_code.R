...
combis_scaled <- expand.grid(
  retransplantation = retransplant,
  lifesupportLifesupport = lifesupport
)
...
df_synth_scaled$lifesupportLifesupport <- combis_scaled$lifesupportLifesupport
...
#                    data = training_ffs, ntree = 300, split.depth = "all.trees",
#                    var.used = "all.trees", seed = -12345, mtry = 12, nodesize = 50,
#                    sampsize = nrow(training_ffs) / 2, forest = TRUE, importance = FALSE
# )
# synth_rsf_ffs <- predictSurvProb(object = model_rsf,
#                                  newdata = df_synth_normal,
#                                  times = times)
...
lines(times, synth_cox_full[2, ], col = "orange", lty = 1 , lwd = 2)
lines(times, synth_rsf_ffs[2, ], col = "orange", lty = 3, lwd  = 2)
lines(times, synth_nn1h[2, ], col = "orange", lty = 2 , lwd = 2)
lines(times, synth_cox_full[3, ], col = "red", lty = 1, lwd  = 2)
lines(times, synth_rsf_ffs[3, ], col = "red", lty = 3 , lwd = 2)
lines(times, synth_nn1h[3, ], col = "red", lty = 2, lwd  = 2)


legend("bottomleft", legend = c("Cox", "RSF", "NN", 
                                "Ref", "On life-support", "Re-trans + On life-support"),
       lty = c(1, 3, 2, 1, 1, 1) , col = c("black", "black", "black",
                                           "green", "orange", "red"),
       bty = "n", cex = 1.0, border = "white", xjust = 0)
